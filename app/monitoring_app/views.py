...
def support(request):
    return render(request, 'monitoring_app/support/support.html')
        ...
        if url_target and page_count:
            ...
            if re.match(r'\d\b', page_count) or re.match(r'\d\d\b', page_count) and not re.match('0', page_count):
                ...
                # If target_url invalid
                    ...
                    return render(request, 'monitoring_app/exceptions/invalid_url.html')

            # If page_count invalid
            else:
                return render(request, 'monitoring_app/exceptions/invalid_page_count.html')

        # If page_count doesn`t exist
        elif url_target and not page_count:
            return render(request, 'monitoring_app/exceptions/not_page_count.html')

        # If url_target doesn`t existstatus_true
        elif page_count and not url_target:
            return render(request, 'monitoring_app/exceptions/not_url_target.html')

        # If nothing exist
        else:
            return render(request, 'monitoring_app/exceptions/not_arguments.html')

        return render(request, 'monitoring_app/success.html', context={'url_target': url_target,
